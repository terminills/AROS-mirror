        ...
        case 2:
            {
                TEXT nametmp[100];
                char *volname;
                if (!GetVolumeForDevName(data->instc_options_main->opt_sysdevname, nametmp))
                {
                    if (data->instc_default_usb)
                ...
                if (!GetVolumeForDevName(data->instc_options_main->opt_workdevname, nametmp))
                {
                    if (data->instc_default_usb)
                        volname = USB_SYS_VOL_NAME;
                    else 
...
IPTR Install__MUIM_Partition(Class * CLASS, Object * self, Msg message)
{
    ...
    if (data->inst_success == MUIV_Inst_InProgress)
    {
        ...
        if (option == 1)
        {
            ...
            D(bug("[InstallAROS] Partitioning Free Space...\n"));
...
IPTR Install__MUIM_IC_Install(Class * CLASS, Object * self, Msg message)
{
    ...
    if (option && (data->inst_success == MUIV_Inst_InProgress))
    {
        ...
        BOOTLOADER_AddSkipPaths(&SKIPLIST);
    ...
    if (option && (data->inst_success == MUIV_Inst_InProgress))
    {
        ...
        if ((lock = Lock(developerDir, SHARED_LOCK)) != NULL)
        {
            ...
            D(bug("[InstallAROS] Couldn't locate Developer Files...\n"));
    ...
    if (!option && (data->inst_success == MUIV_Inst_InProgress))
    {
        ...
        sprintf(packagePath, "%s:Prefs/Env-Archive/SYS/Packages/Developer",
            data->install_SysTarget);
        D(bug("[InstallAROS] Deleting Developer Package...\n"));
