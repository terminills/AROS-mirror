...
struct BootLoaderInfo BootLoaderData[] = {
    ...
    {
        "grub2",
        ARCHBOOTDIR "/grub2/" GRUBARCHDIR "/core.img",
        "c/Install-grub2",
        ARCHBOOTDIR "/grub2/fonts/unicode.pf2"
    },
#endif
    {
        -1,
...
void BOOTLOADER_AddSkipPaths(struct List *SkipList)
{
...
void BOOTLOADER_DoInstall(Class * CLASS, Object * self)
{
    ...
    case BOOTLOADER_GRUB2:
        {
                ...
                && XGET(cycle_drivetype, MUIA_Cycle_Active) != 2)
            {
                ...
                if (!success)
                {
                    ...
                    D(bug
                        ("[InstallAROS] Windows partition found. Adding Windows option to GRUB2 menu.\n"));
                }
            }
    ...
    case BOOTLOADER_GRUB1:
                ...
                FindWindowsPartition(data->bl_TargetDevice, data->bl_TargetUnit)) != -1)
        {
            ...
            D(bug
                ("[InstallAROS] Windows partition found. Adding Windows option to GRUB menu.\n"));
        }
